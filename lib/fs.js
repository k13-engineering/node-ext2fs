...
module.exports = function(instance, fsPointer) {

const util = require('util');
const pathModule = require('path');

const binding = require('./binding')(constants, instance, fsPointer);
const fs = {};
...
ReadStream.prototype.close = function(cb) {
  ...
  if (cb) {
    if (this.closed) {
      ...
      this._closeCallbacks = (this._closeCallbacks || []).concat([cb]);
    }
  }
  ...
  closeOrError((err) => {
    self._closeCallbacks = self._closeCallbacks || [];
    self._closeCallbacks.forEach((cb) => cb(err));
